...
#include <ql/termstructures/volatility/equityfx/localvolsurface.hpp>
//#include <ql/termstructures/volatility/equityfx/localvolsurfacegridded.hpp>
//#include <ds_equity_derivatives/vol_surface/sabr_vol_surface.hpp>
...
namespace QuantLib {
	...
	Real GeneralizedBlackScholesProcess::drift(Time t, Real x, Time dt) const {
		Real sigma = diffusion(t,x);
		Time t1 = t + dt;
		Real rf = riskFreeRate_->forwardRate(t,t1,Continuous,NoFrequency,true);
		Real div = dividendYield_->forwardRate(t,t1,Continuous,NoFrequency,true);
		return (rf - div - 0.5 * sigma * sigma) * dt;
	}
	...
	Real GeneralizedBlackScholesProcess::evolve(Time t0, Real x0,
		Time dt, Real dw) const {
			return apply(x0, drift(t0,x0,dt) +	stdDeviation(t0,x0,dt)*dw);
	}
	...
	const Handle<LocalVolTermStructure>&
		GeneralizedBlackScholesProcess::localVolatility() const {
			if (!updated_) {
				...
				/*
				boost::shared_ptr<SABRBlackVarianceSurface> volSurfaceGridded =
					boost::dynamic_pointer_cast<SABRBlackVarianceSurface>(
					*blackVolatility());

				if(volSurfaceGridded)
				{
					localVolatility_.linkTo(
						boost::shared_ptr<LocalVolTermStructure>(
						new LocalVolSurfaceGridded( volSurfaceGridded, riskFreeRate_,
	...
	/*Added by K.H.Hwang*/
	/*2012.5.31*/
	BSM_QuantoAdjusted_Process::BSM_QuantoAdjusted_Process(
		const Handle<Quote>& x0,
		const Handle<YieldTermStructure>& dividendTS,
		const Handle<YieldTermStructure>& riskFreeTS,
		const Handle<BlackVolTermStructure>& blackVolTS,
		...
		const Real fxCorr,
		const boost::shared_ptr<discretization>& d)
		: GeneralizedBlackScholesProcess(x0,dividendTS,riskFreeTS,blackVolTS,d), fxVol_(fxVol), fxCorr_(fxCorr) {}

	Real BSM_QuantoAdjusted_Process::drift(Time t, Real x, Time dt) const {
		Real sigma = diffusion(t,x);
		Time t1 = t + dt;
		Real rf = riskFreeRate_->forwardRate(t,t1,Continuous,NoFrequency,true);
		Real div = dividendYield_->forwardRate(t,t1,Continuous,NoFrequency,true);
		return (rf - div - 0.5 * sigma * sigma - sigma * fxVol_ * fxCorr_) * dt;
	}

	Real BSM_QuantoAdjusted_Process::expectation(Time t, Real x, Time dt) const {
		return logExpectation(t,x,dt);
	}

	Real BSM_QuantoAdjusted_Process::logExpectation(Time t, Real x, Time dt) const {
		return std::log(x) + drift(t,x,dt);
	}
