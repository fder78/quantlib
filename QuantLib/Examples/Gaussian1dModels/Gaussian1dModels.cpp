...
void printBasket(const std::vector<boost::shared_ptr<CalibrationHelper> > &basket) {
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        std::cout
            << "\nThe discounting curve: Eonia curve at a level of " << oisLevel
            << "\nThe forwarding curve : Euribior 6m curve at a level of " << forward6mLevel << std::endl;
        ...
        std::cout
            << "\nFlat swaption volatility at "
            << volLevel << std::endl;

        waitForKey();

        Real strike = 0.04;
        std::cout << "\nStandard 10y bermudan payer swaption with yearly exercises at a strike of " << strike << std::endl;
		...
		std::cout<<"(1) ATM swaptions"<<std::endl;
        //std::cout
        ...
        std::cout << "\nBermudan swaption NPV (ATM calibrated GSR) = " << std::fixed << std::setprecision(6) << npv << std::endl;
        printTiming(timer);

		std::cout<<"(2) Mat Stk By DeltaGamma"<<std::endl;
        //std::cout
        ...
        std::cout << "\nBermudan swaption NPV (deal strike calibrated GSR) = " << std::setprecision(6) << npv << std::endl;

        printTiming(timer);

		std::cout<<"(3) Amortizing Swap"<<std::endl;
        std::cout
        ...
        boost::shared_ptr<NonstandardSwap> underlying2(new NonstandardSwap(
            ...
            Actual360()));
        boost::shared_ptr<NonstandardSwaption> swaption2 = boost::make_shared<NonstandardSwaption>(underlying2, exercise);

        swaption2->setPricingEngine(nonstandardSwaptionEngine);

        waitForKey();
        timer.start();
        basket = swaption2->calibrationBasket(swapBase, *swaptionVol, BasketGeneratingEngine::MaturityStrikeByDeltaGamma);
        timer.stop();
        ...
        std::vector<Real> nominalFloating2(nominalFloating.size(), 0.0); // null the second leg
        ...
        boost::shared_ptr<RebatedExercise> exercise2 = boost::make_shared<RebatedExercise>(*exercise, -1.0, 2, TARGET());

        boost::shared_ptr<NonstandardSwaption> swaption3 = boost::make_shared<NonstandardSwaption>(underlying3, exercise2);

        boost::shared_ptr<SimpleQuote> oas0 = boost::make_shared<SimpleQuote>(0.0);
        boost::shared_ptr<SimpleQuote> oas100 = boost::make_shared<SimpleQuote>(0.01);
        RelinkableHandle<Quote> oas(oas0);

        boost::shared_ptr<PricingEngine> nonstandardSwaptionEngine2 = 
			boost::make_shared<Gaussian1dNonstandardSwaptionEngine>(gsr, 64, 7.0, true, false, oas); // change discounting to 6m
		...
		std::cout<<"(6) CMS 10Y Underlying SWAP"<<std::endl;
        ...
        boost::shared_ptr<Gaussian1dFloatFloatSwaptionEngine> floatSwaptionEngine =
            boost::make_shared<Gaussian1dFloatFloatSwaptionEngine>(gsr, 64, 7.0, true, false, Handle<Quote>(), ytsOis, true);
        ...
        boost::shared_ptr<CmsCouponPricer> cmsPricer = boost::make_shared<LinearTsrPricer>(swaptionVol, reversionQuote);
        boost::shared_ptr<IborCouponPricer> iborPricer = boost::make_shared<BlackIborCouponPricer>();

        setCouponPricer(leg0, cmsPricer);
        setCouponPricer(leg1, iborPricer);

        boost::shared_ptr<PricingEngine> swapPricer = boost::make_shared<DiscountingSwapEngine>(ytsOis);
		...
		std::cout<<"(7) ATM Calibration"<<std::endl;
		...
		std::cout<<"(8) Underlying SWAP in GSR"<<std::endl;
