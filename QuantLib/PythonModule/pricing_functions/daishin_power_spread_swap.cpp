#include "StdAfx.h"
#include "daishin_power_spread_swap.hpp"
	...
	std::vector<Real> power_spread_swap(Date evaluationDate,
		...
		Size numCalibration) {
			...
			Leg floatingcashflows = IborLeg(floatingSchedule, index1)
				...
				.withPaymentDayCounter(floatingDayCounter) //floatingDayCounter)
				.withPaymentAdjustment(bdc)
				.withSpreads(alpha)
				.withGearings(floatingGearing)
				.withPaymentAdjustment(bdc);

			PowerSpreadSwap testProduct(
				0, notional, side, floatingSchedule, 
				floatingcashflows, alpha, 
