...
tree = ET.parse('test.trn')
root = tree.getroot()
data_root = root.find("param_root").find("data_root")

#for temp in data_root.findall("record"):
#    temp.find("CalcDelta").set("value", "N")
#    temp.find("CalcChunkDelta").set("value", "N")
#    temp.find("CalcVega").set("value", "N")
#    head = ET.SubElement(temp, "CalcXGamma")
#    head.set("type", "string")
#    head.set("value", "N")

input = ET.tostring(root, method='xml')
QL = pm.QL(input)
